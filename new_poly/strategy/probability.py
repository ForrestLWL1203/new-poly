...
def binary_probability(s_price: float, k_price: float, sigma: float, seconds_left: float) -> float:
    ...
    # Numerical guard only: the live strategy stops opening new positions far
    # before expiry and final-force exits handle the last seconds. Keeping a
    # tiny positive T avoids unstable d2 values without changing post-expiry
    # settlement behavior.
    t_years = max(seconds_left, MIN_SECONDS_LEFT_FOR_D2) / SECONDS_PER_YEAR
