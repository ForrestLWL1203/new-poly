...
def test_paper_sell_uses_live_style_exit_floor(monkeypatch) -> None:
    ...
    async def scenario() -> None:
        ...
        result = await gateway.sell("up", shares=10.0, min_price=0.40, exit_reason="poly_hold_score_exit")
...
def test_live_sell_profit_exit_uses_configured_aggressive_retry(monkeypatch) -> None:
    ...
    result = asyncio.run(gateway.sell("up", shares=10.0, min_price=0.40, exit_reason="poly_hold_score_exit"))
...
def test_live_sell_retry_refreshes_exit_floor_before_second_post(monkeypatch) -> None:
    ...
    async def refresh_retry(attempt):
        ...
        if attempt == 0:
            return SellRetryParams(min_price=0.40, exit_reason="poly_hold_score_exit")
        if attempt == 1:
            return SellRetryParams(min_price=0.50, exit_reason="poly_hold_score_exit")
    ...
    result = asyncio.run(
        gateway.sell(
            ...
            shares=10.0,
            min_price=0.40,
            exit_reason="poly_hold_score_exit",
...
def test_live_batch_sell_posts_multiple_fak_slices(monkeypatch) -> None:
    ...
    result = asyncio.run(gateway.sell("up", shares=100.0, min_price=0.38, exit_reason="poly_hold_score_exit"))
...
def test_live_sell_balance_error_is_not_fatal(monkeypatch) -> None:
    ...
    result = asyncio.run(gateway.sell("up", shares=4.0, min_price=0.10, exit_reason="poly_hold_score_exit"))
...
def test_live_sell_request_exception_reconciles_from_balance_and_trades(monkeypatch) -> None:
    ...
    result = asyncio.run(gateway.sell("up", shares=4.0, min_price=0.12, exit_reason="poly_hold_score_exit"))
...
def test_live_sell_success_trusts_response_when_balance_lags(monkeypatch) -> None:
    ...
    result = asyncio.run(gateway.sell("down", shares=2.941175, min_price=0.11, exit_reason="poly_hold_score_exit"))
...
def test_live_sell_dust_shares_are_not_posted(monkeypatch) -> None:
    ...
    result = asyncio.run(gateway.sell("up", shares=0.005787, min_price=0.64, exit_reason="poly_hold_score_exit"))
...
def test_live_post_service_not_ready_is_no_fill_not_crash(monkeypatch) -> None:
    class Client:
        def create_market_order(self, args, options=None):
            return {"signed": True}

        def post_order(self, signed, order_type):
            raise RuntimeError("PolyApiException[status_code=425, error_message=service not ready]")
    ...
    result = gateway._post("up", 1.0, "SELL", 0.50)
...
def test_paper_sell_uses_local_tick_without_clob_lookup(monkeypatch) -> None:
    ...
    async def scenario() -> None:
        ...
        result = await gateway.sell("up", shares=10.0, min_price=0.50, exit_reason="poly_hold_score_exit")
...
def test_paper_batch_sell_can_partially_exit_large_share_position(monkeypatch) -> None:
    ...
    async def scenario() -> None:
        ...
        result = await gateway.sell("up", shares=100.0, min_price=0.38, exit_reason="poly_hold_score_exit")
